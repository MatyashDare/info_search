...
def main():   
    st.title('🥰')
#     video_url = 'https://youtu.be/XMAn_DqXmRM'
#     video_url  = 'https://www.youtube.com/watch?v=OGnDljE4R2g'
#     st.video(video_url)
    audio_file = open('audio.wav', 'rb')
    audio_bytes = audio_file.read()
    st.audio(audio_bytes, format='audio/wav')
    st.title('Проблема с личной жизнью? Может, здесь получится найти ответ, близкий сердцу?')
    st.header('корпус составлен на ответах Мейл.ру')
    st.subheader('он смешной')
    st.write('ниже можно ввести ваш запрос')
    ...
    top_n = st.slider('Выберите число топ-н результатов поиска', 1, 26, 5)
    
    if st.button('искать'):
        ans_time = -1000
        ans, ans_time = query_return(query, choice, top_n)
        ans_df = pd.DataFrame({'ответы': ans})
        st.write('Вот, что удалось найти:')
        st.dataframe(ans_df)
        st.success(f"Все ответы нашлись за {ans_time} секунд!" )
        if ans_time != -1000:
            st.balloons()
